...
"""%("TRUE" if stress else "FALSE",BL,BL,BL)
    with open("/mnt/ntfs2/ethanol_nnp/aimd/train/etoh_train.qe","w") as fp:
       fp.write(fl)
       for i in range(NMOL*NATM):
           fp.write("%s %.15e %.15e %.15e\n"%(elms[i%NATM],cod[i,0],cod[i,1],cod[i,2]))
    if os.system("export OMP_NUM_THREADS=1 && cd /mnt/ntfs2/ethanol_nnp/aimd/train && mpiexec -n 32 /home/yuansheng/quantum_espresso/qe-7.0_cpu/build/bin/pw.x -i etoh_train.qe > res.log"): raise ValueError("qe failed!")
def save_qe():
    ff,e,prs=qe_read("/mnt/ntfs2/ethanol_nnp/aimd/train/res.log")
    if prs is not None: 
        np.savez("/mnt/ntfs2/ethanol_nnp/aimd/train_data/qe_%f_%d.npz"%(BL,time.time()),x=cod,f=ff,e=e,p=prs)
        print("erg =",float(e),"Ry; prs =",float(prs),"Kbar")
    else:               
        np.savez("/mnt/ntfs2/ethanol_nnp/aimd/train_data/qe_%f_%d.npz"%(BL,time.time()),x=cod,f=ff,e=e)
...
if __name__=="__main__":
    ...
    els=[]
    flst=glob.glob("/mnt/ntfs2/ethanol_nnp/aimd/train_data/qe_*_*.npz")
    for f in flst:
    ...
    flst=glob.glob("/mnt/ntfs2/ethanol_nnp/aimd/train_data/qe_%f*.npz"%(BL))
    ...
    with open("/mnt/ntfs2/ethanol_nnp/nnp_data/list_300_%.4f.txt"%(BL),"w") as fp:
        fp.write(str(len(f_train))+"\n")
        for fn in f_train:
            fp.write(fn+".bin\n")
    with open("/mnt/ntfs2/ethanol_nnp/nnp_data/listeval_300_%.4f.txt"%(BL),"w") as fp:

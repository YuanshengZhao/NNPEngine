...
def save_qe():
    ...
    if prs is not None: 
        np.savez("/mnt/ntfs2/glycerol_nnp/aimd/train_data/qe_%f_%d.npz"%(BL,time.time()),x=cod,f=ff,e=e,p=prs)
        print("erg =",float(e),"Ry; prs =",float(prs),"Kbar")
    else:               
        np.savez("/mnt/ntfs2/glycerol_nnp/aimd/train_data/qe_%f_%d.npz"%(BL,time.time()),x=cod,f=ff,e=e)
